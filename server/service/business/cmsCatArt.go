...
func (m *CmsCatArtService) GetListAct(ctx context.Context, seq business.CmsCatArtSearch) (list []business.CmsCatArt, total int64, err error) {
	...
	OrderStr := "a.id desc"
	if seq.BeHot != nil {
		db = db.Where("be_hot = ?", seq.BeHot)
	}
	err = db.Select(fields).Order(OrderStr).Scan(&list).Error
	return list, 0, err
}
