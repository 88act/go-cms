...
func (s *CmsCatRouter) InitCmsCatRouter(Router *gin.RouterGroup) {
	...
	var apiV1 = v1.ApiGroupApp.BusinessApiGroup.CmsCatApi
	{
		...
		router.POST("deleteCmsCat", apiV1.DeleteCmsCat) // 删除CmsCat
		router.POST("deleteCmsCatByIds", apiV1.DeleteCmsCatByIds) // 批量删除CmsCat
		router.POST("updateCmsCat", apiV1.UpdateCmsCat)    // 更新CmsCat
