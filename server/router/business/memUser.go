...
func (s *MemUserRouter) InitMemUserRouter(Router *gin.RouterGroup) {
	...
	var apiV1 = v1.ApiGroupApp.BusinessApiGroup.MemUserApi
	{
		...
		router.POST("deleteMemUser", apiV1.DeleteMemUser)           // 删除MemUser
		router.POST("deleteMemUserByIds", apiV1.DeleteMemUserByIds) // 批量删除MemUser
		router.POST("updateMemUser", apiV1.UpdateMemUser)           // 更新MemUser
		router.POST("quickEdit", apiV1.QuickEdit)                   // 快速编辑
		router.POST("memChangePwd", apiV1.MemChangePwd)             // 快速编辑
