      ...
      <el-table-column type="selection" width="55" />      
       <el-table-column label="ID" min-width="60" prop="ID" sortable="custom" />
        ...
        <template #default="scope">
          ...
          <!--bzksdm  BeQuickEdit -->  
        <el-table-column label="标准科室代码" prop="bzksdm" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.bzksdm"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('bzksdm',scope.row.ID,scope.row.bzksdm,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.bzksdm}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
          <!--ybjdm  BeQuickEdit -->  
        <el-table-column label="医保局代码" prop="ybjdm" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.ybjdm"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('ybjdm',scope.row.ID,scope.row.ybjdm,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.ybjdm}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
          <!--ksjs  BeQuickEdit -->  
        <el-table-column label="科室简介" prop="ksjs" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.ksjs"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('ksjs',scope.row.ID,scope.row.ksjs,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.ksjs}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
...
import {
  ...
  deleteBbPiDepartmentByIds,
  updateBbPiDepartment,
  findBbPiDepartment,
  getBbPiDepartmentList,
  quickEdit,
  excelList
...
export default {
  ...
  methods: { 
    ...
    async onDelete() {
  	...
  	async doDelete(ids) { 
     const res = await deleteBbPiDepartmentByIds({ ids })
      if (res.code === 200) {
        this.$message({
          type: 'success',
          message: '删除成功'
   ...
   async goEditForm(id) { 
	  if (this.beNewWindow) {
		  if (id >0) {
			this.$router.push({ name:'bbPiDepartmentForm', params:{id:id}})
		  } else {
			 this.$router.push({ name:'bbPiDepartmentForm',params:{id:id}})
		  }
	  }else
