        ...
        <el-form-item label="ID">
            <el-input placeholder="搜索ID" v-model="searchInfo.ID" />
                ...
                <el-form-item label="字段名">
                  <el-input placeholder="搜索条件" v-model="searchInfo.tabField" clearable />
                </el-form-item>
        <el-form-item>
    ...
    >
      <el-table-column type="selection" width="55" />      
       <el-table-column label="ID" min-width="60" prop="ID" sortable="custom" />
          <!--status  BeQuickEdit -->
        <el-table-column label="是否上传" prop="status" width="120"  sortable="custom" >
        ...
        <template #default="scope">
          ...
          <!--tabNameCn  BeQuickEdit -->  
        <el-table-column label="表名cn" prop="tabNameCn" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.tabNameCn"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('tab_name_cn',scope.row.ID,scope.row.tabNameCn,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.tabNameCn}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
          <!--tabField  BeQuickEdit -->  
        <el-table-column label="字段名" prop="tabField" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.tabField"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('tab_field',scope.row.ID,scope.row.tabField,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.tabField}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
          <!--tabFieldCn  BeQuickEdit -->  
        <el-table-column label="字段名cn" prop="tabFieldCn" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.tabFieldCn"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('tab_field_cn',scope.row.ID,scope.row.tabFieldCn,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.tabFieldCn}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
...
export default {
  ...
  methods: { 
    ...
    async onDelete() {
  	...
  	async doDelete(ids) { 
     const res = await deleteBbPiUpFieldByIds({ ids })
      if (res.code === 200) {
        this.$message({
          type: 'success',
          message: '删除成功'
   ...
   async goEditForm(id) { 
	  if (this.beNewWindow) {
		  if (id >0) {
			this.$router.push({ name:'bbPiUpFieldForm', params:{id:id}})
		  } else {
			 this.$router.push({ name:'bbPiUpFieldForm',params:{id:id}})
		  }
	  }else
