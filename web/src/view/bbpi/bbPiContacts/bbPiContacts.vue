      ...
      <el-table-column type="selection" width="55" />      
       <el-table-column label="ID" min-width="60" prop="ID" sortable="custom" />
        ...
        <template #default="scope">
          ...
          <!--bm  BeQuickEdit -->  
        <el-table-column label="部门" prop="bm" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.bm"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('bm',scope.row.ID,scope.row.bm,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.bm}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
          <!--fzrxm  BeQuickEdit -->  
        <el-table-column label="负责人姓名" prop="fzrxm" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.fzrxm"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('fzrxm',scope.row.ID,scope.row.fzrxm,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.fzrxm}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
          <!--lxdh  BeQuickEdit -->  
        <el-table-column label="联系电话" prop="lxdh" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.lxdh"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('lxdh',scope.row.ID,scope.row.lxdh,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.lxdh}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
    ...
    <el-dialog  v-if="dialogFormVisible"  :before-close="closeDialog" v-model="dialogFormVisible" title="编辑资料">
      <el-form :model="formData" label-position="right" label-width="80px">
        ...
        <el-form-item label="部门:"> 
              <el-input v-model="formData.bm" clearable placeholder="请输入" />
       </el-form-item>
        <el-form-item label="负责人姓名:"> 
              <el-input v-model="formData.fzrxm" clearable placeholder="请输入" />
       </el-form-item>
...
import {
  createBbPiContacts, 
  deleteBbPiContactsByIds,
  updateBbPiContacts,
  findBbPiContacts,
  getBbPiContactsList,
  quickEdit,
  excelList
} from '@/api/bbPiContacts' //  此处请自行替换地址
...
export default {
  ...
  methods: { 
    ...
    async onDelete() {
  	...
  	async doDelete(ids) { 
     const res = await deleteBbPiContactsByIds({ ids })
      if (res.code === 200) {
        this.$message({
          type: 'success',
          message: '删除成功'
   ...
   async goEditForm(id) { 
	  if (this.beNewWindow) {
		  if (id >0) {
			this.$router.push({ name:'bbPiContactsForm', params:{id:id}})
		  } else {
			 this.$router.push({ name:'bbPiContactsForm',params:{id:id}})
		  }
	  }else
	  {
		 if (id >0) {
			  const res = await findBbPiContacts({ID:id})
			  //console.log(res.data)
			  this.editType = 'update'
			  if (res.code === 200) 
			     this.formData = res.data.bbPiContacts 
		 }else
		 {
    ...
    async saveEditForm() {  
      ...
      switch (this.editType) {
        case "create":         
          res = await createBbPiContacts(this.formData);
          break
        case "update": 
          res = await updateBbPiContacts(this.formData);
          break
        default: 
          res = await createBbPiContacts(this.formData);
          break
