        ...
        <el-form-item label="ID">
            <el-input placeholder="搜索ID" v-model="searchInfo.ID" />
                ...
                <el-form-item label="标识">
                  <el-input placeholder="搜索条件" v-model="searchInfo.toKey" clearable />
                </el-form-item> 
                <el-form-item label="字段">
                  <el-input placeholder="搜索条件" v-model="searchInfo.toField" clearable />
                </el-form-item>
      ...
      <el-table-column type="selection" width="55" />      
       <el-table-column label="ID" min-width="60" prop="ID" sortable="custom" />
        ...
        <template #default="scope">
            ...
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.colId"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('col_id',scope.row.ID,scope.row.colId,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.colId}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
          <!--toKey  BeQuickEdit -->  
        <el-table-column label="标识" prop="toKey" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.toKey"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('to_key',scope.row.ID,scope.row.toKey,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.toKey}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
          <!--toField  BeQuickEdit -->  
        <el-table-column label="字段" prop="toField" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.toField"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('to_field',scope.row.ID,scope.row.toField,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.toField}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
...
export default {
  ...
  methods: { 
    ...
    async onDelete() {
  	...
  	async doDelete(ids) { 
     const res = await deleteColKeyFieldByIds({ ids })
      if (res.code === 200) {
        this.$message({
          type: 'success',
          message: '删除成功'
   ...
   async goEditForm(id) { 
	  if (this.beNewWindow) {
		  if (id >0) {
			this.$router.push({ name:'colKeyFieldForm', params:{id:id}})
		  } else {
			 this.$router.push({ name:'colKeyFieldForm',params:{id:id}})
		  }
	  }else
