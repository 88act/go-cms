      ...
      <el-form-item label="创建时间">
            <el-date-picker 
                ...
                <el-form-item label="应用">
                  <el-input placeholder="搜索条件" v-model="searchInfo.deployment" clearable />
                </el-form-item>
        <el-form-item>
       ...
       <el-table-column label="ID" min-width="60" prop="ID" sortable="custom" />
        ...
        <template #default="scope">
            ...
            <el-row :gutter="10">
              ...
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('namespace',scope.row.ID,scope.row.namespace,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
          ...
          <!--deployment  BeQuickEdit -->  
        <el-table-column label="应用" prop="deployment" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.deployment"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('deployment',scope.row.ID,scope.row.deployment,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.deployment}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column> 
          <el-table-column label="实例数" prop="replicas" width="120"   sortable="custom"  /> 
...
export default {
  ...
  methods: { 
    ...
    async onDelete() {
  	...
  	async doDelete(ids) { 
     const res = await deleteK8sDeploymentsByIds({ ids })
      if (res.code === 200) {
        this.$message({
          type: 'success',
          message: '删除成功'
   ...
   async goEditForm(id) { 
	  if (this.beNewWindow) {
		  if (id >0) {
			this.$router.push({ name:'k8sDeploymentsForm', params:{id:id}})
		  } else {
			 this.$router.push({ name:'k8sDeploymentsForm',params:{id:id}})
		  }
	  }else
