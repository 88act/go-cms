      ...
      <el-form-item label="创建时间">
            <el-date-picker 
                ...
                <el-form-item label="集群名称">
                  <el-input placeholder="搜索条件" v-model="searchInfo.clusterName" clearable />
                </el-form-item>
        <el-form-item>
    ...
    >
      <el-table-column type="selection" width="55" />      
       <el-table-column label="ID" min-width="60" prop="ID" sortable="custom" />
          <!--clusterName  BeQuickEdit -->  
        <el-table-column label="集群名称" prop="clusterName" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.clusterName"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('cluster_name',scope.row.ID,scope.row.clusterName,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.clusterName}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
          <!--kubeConfig  BeQuickEdit -->  
        <el-table-column label="Kubeconfig内容" prop="kubeConfig" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.kubeConfig"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('kube_config',scope.row.ID,scope.row.kubeConfig,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.kubeConfig}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
          <!--clusterVersion  BeQuickEdit -->  
        <el-table-column label="集群版本" prop="clusterVersion" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.clusterVersion"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('cluster_version',scope.row.ID,scope.row.clusterVersion,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.clusterVersion}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column> 
...
export default {
  ...
  methods: { 
    ...
    async onDelete() {
  	...
  	async doDelete(ids) { 
     const res = await deleteK8sClustersByIds({ ids })
      if (res.code === 200) {
        this.$message({
          type: 'success',
          message: '删除成功'
   ...
   async goEditForm(id) { 
	  if (this.beNewWindow) {
		  if (id >0) {
			this.$router.push({ name:'k8sClustersForm', params:{id:id}})
		  } else {
			 this.$router.push({ name:'k8sClustersForm',params:{id:id}})
		  }
	  }else
