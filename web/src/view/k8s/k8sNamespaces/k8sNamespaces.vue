      ...
      <el-table-column type="selection" width="55" />      
       <el-table-column label="ID" min-width="60" prop="ID" sortable="custom" />
        ...
        <template #default="scope">
          ...
          <!--createTime  BeQuickEdit -->  
        <el-table-column label="创建时间" prop="createTime" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.createTime"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('create_time',scope.row.ID,scope.row.createTime,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.createTime}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column> 
...
export default {
  ...
  methods: { 
   ...
   async goEditForm(id) { 
	  if (this.beNewWindow) {
		  if (id >0) {
			this.$router.push({ name:'k8sNamespacesForm', params:{id:id}})
		  } else {
			 this.$router.push({ name:'k8sNamespacesForm',params:{id:id}})
		  }
	  }else
