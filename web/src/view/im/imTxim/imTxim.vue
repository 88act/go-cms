<template>
	<div>
		...
		<div class="search-term">
			<el-form :inline="true" :model="searchInfo" class="demo-form-inline">
				...
				<el-form-item label="开始时间">
					<el-date-picker v-model="formData.beginTime" type="datetimerange" format="YYYY-MM-DD HH:mm:ss"
						:shortcuts="shortcuts" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" />
				</el-form-item>
				<el-form-item label="当前时间">
					<el-date-picker v-model="formData.nowTime" type="datetimerange" format="YYYY-MM-DD HH:mm:ss"
						:shortcuts="shortcuts" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" />
				</el-form-item>
				...
				<el-form-item label="运行状态" prop="statusRun">
					<el-select v-model="searchInfo.statusRun" placeholder="请选择" clearable>
						<el-option v-for="(item,key) in status_runOptions" :key="key" :label="item.label"
							:value="item.value"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item>
					...
					<el-popover v-model:visible="deleteVisible" placement="top" width="160">
						...
						<template #reference>
							<el-button icon="el-icon-delete" size="mini" type="danger" style="margin-left: 10px;">批量删除
							</el-button>
						</template>
					</el-popover>
				</el-form-item>
		...
		<!----------数据表------------------ -->
		<el-table ref="multipleTable" border stripe style="width: 100%" tooltip-effect="dark" :data="tableData"
			...
			<el-table-column label="名称" prop="name" width="120">
				<template #default="scope">
					<el-popover trigger="click" placement="top" width="280">
						<el-row :gutter="10">
							<el-col :span="16">
								<el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.name">
								</el-input>
							</el-col>
							<el-col :span="4">
								<el-button size="small" type="primary" icon="el-icon-edit" class="table-button"
									@click="quickEdit_do('name',scope.row.ID,scope.row.name,scope)">保存</el-button>
							</el-col>
						</el-row>
						<template #reference>
			...
			<el-table-column label="appid" prop="appId" width="120" sortable="custom">
				<template #default="scope">
					<el-popover trigger="click" placement="top" width="280">
						<el-row :gutter="10">
							<el-col :span="16">
								<el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.appId">
								</el-input>
							</el-col>
							<el-col :span="4">
								<el-button size="small" type="primary" icon="el-icon-edit" class="table-button"
									@click="quickEdit_do('app_id',scope.row.ID,scope.row.appId,scope)">保存</el-button>
							</el-col>
						</el-row>
						<template #reference>
			...
			<el-table-column label="管理员帐号" prop="identifier" width="120">
				<template #default="scope">
					<el-popover trigger="click" placement="top" width="280">
						<el-row :gutter="10">
							<el-col :span="16">
								<el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.identifier">
								</el-input>
							</el-col>
							<el-col :span="4">
								<el-button size="small" type="primary" icon="el-icon-edit" class="table-button"
									@click="quickEdit_do('identifier',scope.row.ID,scope.row.identifier,scope)">保存
								</el-button>
							</el-col>
						</el-row>
						<template #reference>
			...
			<el-table-column label="开始时间" width="180" prop="beginTime" sortable="custom">
				<template #default="scope">{{formatDate(scope.row.beginTime)}}</template>
			</el-table-column>
			<el-table-column label="当前时间" width="180" prop="nowTime" sortable="custom">
				<template #default="scope">{{formatDate(scope.row.nowTime)}}</template>
			</el-table-column>
			<!--status  BeQuickEdit -->
			...
			<el-table-column label="运行状态" prop="statusRun" width="120" sortable="custom">
				<template #default="scope">
					<el-popover trigger="click" placement="top" width="280">
						<el-select v-model="scope.row.statusRun" placeholder="请选择"
							@change="quickEdit_do('status_run',scope.row.ID,scope.row.statusRun,scope)">
							<el-option v-for="(item,key) in status_runOptions" :key="key" :label="item.label"
								:value="item.value"></el-option>
						</el-select>
						<template #reference>
			...
			<el-table-column label="操作">
				<template #default="scope">
					...
					<el-button plain size="mini" type="primary" icon="el-icon-edit" class="table-button"
						@click="goEditForm(scope.row.ID)">编辑</el-button>
					<el-button plain size="mini" type="danger" icon="el-icon-delete" @click="deleteRow(scope.row)">删除
					</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog v-if="dialogFormVisible" :before-close="closeDialog" v-model="dialogFormVisible" title="编辑资料">
			<el-form :model="formData" label-position="right" label-width="80px">
				...
				<el-form-item label="开始时间:">
					<el-date-picker v-model="formData.beginTime" type="datetime" style="width:100%" placeholder="选择时间日期"
						clearable />
				</el-form-item>
				<el-form-item label="当前时间:">
					<el-date-picker v-model="formData.nowTime" type="datetime" style="width:100%" placeholder="选择时间日期"
						clearable />
				</el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async doDelete(ids) {
				const res = await deleteImTximByIds({
					ids
				})
				if (res.code === 200) {
					this.$message({
