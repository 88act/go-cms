        ...
        <el-form-item label="ID">
            <el-input placeholder="搜索ID" v-model="searchInfo.ID" />
                ...
                <el-form-item label="名称">
                  <el-input placeholder="搜索条件" v-model="searchInfo.areaname" clearable />
                </el-form-item>
                  <el-form-item label="父栏目">
                      <el-input placeholder="搜索条件" v-model="searchInfo.parentid" clearable />
                  </el-form-item>
                  <el-form-item label="级别">
                      <el-input placeholder="搜索条件" v-model="searchInfo.level" clearable />
                  </el-form-item>
    ...
    >
      <el-table-column type="selection" width="55" />      
       <el-table-column label="ID" min-width="60" prop="ID" sortable="custom" /> 
          <el-table-column label="名称" prop="areaname" width="120"   sortable="custom"  /> 
          <el-table-column label="父栏目" prop="parentid" width="120"   sortable="custom"  /> 
          <el-table-column label="简称" prop="shortname" width="120"    /> 
          <el-table-column label="电话区号" prop="areacode" width="120"    /> 
          <el-table-column label="邮编" prop="zipcode" width="120"    /> 
          <el-table-column label="拼音" prop="pinyin" width="120"    /> 
          <el-table-column label="拼音简写" prop="py" width="120"    /> 
          <el-table-column label="经度" prop="lng" width="120"    /> 
          <el-table-column label="纬度" prop="lat" width="120"    />
          <!--level  BeQuickEdit -->  
        <el-table-column label="级别" prop="level" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.level"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('level',scope.row.ID,scope.row.level,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.level}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column> 
          <el-table-column label="position字段" prop="position" width="120"    /> 
          <el-table-column label="组合名称" prop="mergername" width="120"    />
          <!--sort  BeQuickEdit -->  
...
export default {
  ...
  methods: { 
    ...
    async onDelete() {
  	...
  	async doDelete(ids) { 
     const res = await deleteBasicAreaByIds({ ids })
      if (res.code === 200) {
        this.$message({
          type: 'success',
          message: '删除成功'
   ...
   async goEditForm(id) { 
	  if (this.beNewWindow) {
		  if (id >0) {
			this.$router.push({ name:'basicAreaForm', params:{id:id}})
		  } else {
			 this.$router.push({ name:'basicAreaForm',params:{id:id}})
		  }
	  }else
