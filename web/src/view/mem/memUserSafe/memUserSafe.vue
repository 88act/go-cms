    ...
    >
      <el-table-column type="selection" width="55" />      
       <el-table-column label="ID" min-width="60" prop="ID" sortable="custom" /> 
        ...
        <el-table-column label="类型" prop="type" width="120"  sortable="custom" >
          <template #default="scope">
            {{filterDict(scope.row.type,"usersafe_type")}}
          </template>
        </el-table-column> 
          <el-table-column label="旧值" prop="valOld" width="120"    /> 
          <el-table-column label="新值" prop="valNew" width="120"    />
          <!--status  BeQuickEdit -->
...
export default {
  ...
  methods: { 
    ...
    async onDelete() {
  	...
  	async doDelete(ids) { 
     const res = await deleteMemUserSafeByIds({ ids })
      if (res.code === 200) {
        this.$message({
          type: 'success',
          message: '删除成功'
   ...
   async goEditForm(id) { 
	  if (this.beNewWindow) {
		  if (id >0) {
			this.$router.push({ name:'memUserSafeForm', params:{id:id}})
		  } else {
			 this.$router.push({ name:'memUserSafeForm',params:{id:id}})
		  }
	  }else
