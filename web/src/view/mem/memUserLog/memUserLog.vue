            ...
            <el-input placeholder="搜索ID" v-model="searchInfo.ID" />
                ...
                <el-form-item label="类型" prop="type">                
                    <el-select v-model="searchInfo.type" placeholder="请选择" clearable>
                      <el-option v-for="(item,key) in userlog_typeOptions" :key="key" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
      ...
      <el-table-column type="selection" width="55" />      
       <el-table-column label="ID" min-width="60" prop="ID" sortable="custom" /> 
        ...
        <el-popover trigger="click" placement="top"  width = "280">  
              ...
              <template #reference>
                  <div class="quickEdit" > {{filterDict(scope.row.type,"userlog_type")}} </div>
              </template>
            </el-popover>
        </template>  
        </el-table-column>
...
export default {
  ...
  methods: { 
    ...
    async onDelete() {
  	...
  	async doDelete(ids) { 
     const res = await deleteMemUserLogByIds({ ids })
      if (res.code === 200) {
        this.$message({
          type: 'success',
          message: '删除成功'
   ...
   async goEditForm(id) { 
	  if (this.beNewWindow) {
		  if (id >0) {
			this.$router.push({ name:'memUserLogForm', params:{id:id}})
		  } else {
			 this.$router.push({ name:'memUserLogForm',params:{id:id}})
		  }
	  }else
