  ...
  <div class="gocms-form-box">
    <el-form ref="editForm" :model="formData" :rules="editRules" label-position="right" label-width="80px">
      ...
      <el-form-item label="类型:" prop="safeType">
        <el-select v-model="formData.safeType" placeholder="请选择" clearable>
          <el-option v-for="(item,key) in usersafe_type_options" :key="key" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="旧值:" prop="valOld">
        <el-input v-model="formData.valOld" clearable placeholder="请输入" />
      </el-form-item>
      <el-form-item label="新值:" prop="valNew">
        <el-input v-model="formData.valNew" clearable placeholder="请输入" />
      </el-form-item>
      <el-form-item label="ip:" prop="ip">
        <el-input v-model.number="formData.ip" clearable placeholder="请输入" />
      </el-form-item>
      <el-form-item label="ip城市:" prop="ipAddr">
...
<script setup lang="ts">
  ...
  const getData = async () => {
    ...
    const res = await findMemUserSafe({
      id: id.value
    })
    if (res.code === 200) {
  ...
  const save = async () => {
    ...
    if (resValid) {
      ...
      if (id.value > 0) { //update
        res = await updateMemUserSafe(formData.value)
      } else {
        formData.value.status = 1
        res = await createMemUserSafe(formData.value)
      }
      if (res.code === 200) {
        console.log(res)
        beChange.value = true;
