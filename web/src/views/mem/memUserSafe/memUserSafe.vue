  ...
  <div>
    ...
    <div class="gocms-table-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        ...
        <div v-if="searchToggle" class="gocms-box-search">
          ...
          <el-form-item label="类型" prop="safeType">
            <el-select v-model="searchInfo.safeType" placeholder="请选择" clearable>
              <el-option v-for="(item,key) in usersafe_type_options" :key="key" :label="item.label"
                :value="item.value"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="旧值">
            <el-input placeholder="搜索条件" v-model="searchInfo.valOld" clearable />
          </el-form-item>
          <el-form-item label="新值">
            <el-input placeholder="搜索条件" v-model="searchInfo.valNew" clearable />
          </el-form-item>
          <el-form-item label="ip">
            <el-input placeholder="搜索条件" v-model="searchInfo.ip" clearable />
          </el-form-item>
          <el-form-item label="ip城市">
            <el-input placeholder="搜索条件" v-model="searchInfo.ipAddr" clearable />
          </el-form-item>
      ...
      <el-table row-key="id" ref="multipleTable" border style="width: 100%" tooltip-effect="dark" :data="tableData"
        ...
        <el-table-column label="类型" prop="safeType" min-width="120" sortable="custom">
          <template #default="scope">
            <el-popover trigger="click" placement="top" width="280">
              <el-select v-model="scope.row.safeType" placeholder="请选择"
                @change="quickEdit_do('safe_type',scope.row.id,scope.row.safeType,scope)">
                <el-option v-for="(item,key) in usersafe_type_options" :key="key" :label="item.label"
                  :value="item.value"></el-option>
              </el-select>
              <template #reference>
                <div class="quickEdit"> {{filterDict(scope.row.safeType,usersafe_type_options)}} </div>
              </template>
            </el-popover>
          </template>
        </el-table-column>
        <el-table-column label="旧值" prop="valOld" min-width="120" sortable="custom" />
        <el-table-column label="新值" prop="valNew" min-width="120" sortable="custom" />
        <el-table-column label="ip" prop="ip" min-width="120" sortable="custom" />
        <el-table-column label="ip城市" prop="ipAddr" min-width="120" sortable="custom" />
...
<script setup lang="tsx">
  ...
  import {
    createMemUserSafe,
    deleteMemUserSafeByIds,
    updateMemUserSafe,
    findMemUserSafe,
    getMemUserSafeList,
    quickEdit,
    excelList
  } from '@/api/memUserSafe'

  import MemUserSafeForm from './memUserSafeForm.vue'
  ...
  // 字典
  const usersafe_type_options = ref([])
  ...
  const deleteRow = async (row) => {
    ElMessageBox.confirm('确认删除?', '提示', {
      ...
      .then(async () => {
        ...
        const res = await deleteMemUserSafeByIds(data)
        if (res.code === 200) {
          message(res.msg, {
            type: "success"
          })
  ...
  const getTableData = async () => {
    ...
    const res = await getMemUserSafeList(paramData)
    if (res.code === 200) {
  ...
  const getOptionsData = async () => {
    usersafe_type_options.value = await getDict('usersafe_type')
