  ...
  <div>
    ...
    <div class="gocms-table-box bg-bg_color">
        ...
        <div v-if="searchToggle" class="gocms-box-search">
          ...
          <el-form-item label="类型" prop="type">
            <el-select v-model="searchInfo.type" placeholder="请选择" clearable>
              <el-option v-for="(item,key) in userlog_type_options" :key="key" :label="item.label"
      ...
      <el-table row-key="id" ref="multipleTable" border style="width: 100%" tooltip-effect="dark" :data="tableData"
        ...
        <el-table-column label="gocmsid" prop="userId" min-width="120" sortable="custom" />
        <el-table-column label="类型" prop="type" min-width="120" sortable="custom">
          <template #default="scope">
            {{filterDict(scope.row.type,userlog_type_options)}}
          </template>
        </el-table-column>
...
<script setup lang="tsx">
  ...
  import MemLogsForm from './memLogsForm.vue'
  ...
  // 字典 
  const userlog_type_options = ref([])
  ...
  const deleteRow = async (row) => {
    ElMessageBox.confirm('确认删除?', '提示', {
      ...
      .then(async () => {
        ...
        const res = await deleteMemLogsByIds(data)
        if (res.code === 200) {
          message(res.msg, {
            type: "success"
          })
  ...
  const getTableData = async () => {
    ...
    const res = await getMemLogsList(paramData)
    if (res.code === 200) {
  ...
  const getOptionsData = async () => {
    userlog_type_options.value = await getDict('userlog_type')
