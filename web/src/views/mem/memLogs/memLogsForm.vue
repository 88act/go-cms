<template>
  <div class="gocms-form-box">
    <el-form ref="editForm" :model="formData" :rules="editRules" label-position="right" label-width="80px">
      ...
      <el-form-item label="gocmsid:" prop="userId">
      ...
      <el-form-item label="类型:" prop="type">
        <el-select v-model="formData.type" placeholder="请选择" clearable>
          <el-option v-for="(item,key) in userlog_type_options" :key="key" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
  ...
  const save = async () => {
    ...
    if (resValid) {
      ...
      if (id.value > 0) { //update
        res = await updateMemLogs(formData.value)
      } else {
        formData.value.status = 1
        res = await createMemLogs(formData.value)
      }
      if (res.code === 200) {
        console.log(res)
        beChange.value = true;
