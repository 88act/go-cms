<template>
	<div>
		...
		<div class="gocms-table-box bg-bg_color">
			...
			<!----------数据表------------------ -->
			<el-table row-key="id" ref="multipleTable" border  style="width: 100%" tooltip-effect="dark" :data="tableData" @selection-change="handleSelectionChange" @sort-change="sortChange" :header-cell-style="{'text-align':'center'}"
:cell-style="{'text-align':'center'}" >
				<el-table-column label="序号" width="80" prop="id" sortable="custom" />
					<el-table-column label="角色名" prop="roleName" min-width="150"   sortable="custom"  />
					<!-- <el-table-column label="角色编码" prop="roleCode" min-width="120"   sortable="custom"  /> -->
        <!--    <el-table-column label="菜单" prop="menuList" min-width="120"   sortable="custom"  />
					<el-table-column label="默认菜单" prop="defaultMenu" min-width="120"   sortable="custom"  /> -->
          ...
          <el-table-column label="创建时间" min-width="120" prop="created_at" sortable="custom">
 ...
 <script setup lang="tsx">
   ...
   const deleteRow = async (row) => {
     ElMessageBox.confirm('确认删除?', '提示', {
       ...
       .then(async () => {
         ...
         const res = await deleteSysRoleByIds(data)
         if (res.code === 200) {
           message(res.msg, {
             type: "success"
           })
   ...
   const getTableData = async () => {
     ...
     const res = await getSysRoleList(paramData)
     if (res.code == 200) {
