<template>
        ...
        <el-form-item label="api中文描述:"  prop="desc">
        ...
        <el-form-item label="api组:"  prop="apiGroup">
                <el-input v-model="formData.apiGroup" clearable placeholder="请输入" />
       </el-form-item>
  ...
  const save = async () => {
	...
	//图片
      ...
      if (id.value > 0) { //update
        res = await updateSysApis(formData.value)
      } else {
        formData.value.status = 1
        res = await createSysApis(formData.value)
      }
      if (res.code === 200) {
        console.log(res)
        beChange.value = true;
