<template>	 
        ...
        <el-form-item label="类型:"  prop="type">
                 <el-select v-model="formData.type" placeholder="请选择" clearable>
                      <el-option v-for="(item,key) in group_type_options" :key="key" :label="item.label" :value="item.value" />
                 </el-select>
       </el-form-item>
        ...
        <el-form-item label="平台:"  prop="platType">
                 <el-select v-model="formData.platType" placeholder="请选择" clearable>
                      <el-option v-for="(item,key) in plat_type_options" :key="key" :label="item.label" :value="item.value" />
                 </el-select>
       </el-form-item>
        <el-form-item label="客户端ip:"  prop="clientIp">
                 <el-input v-model.number="formData.clientIp" clearable placeholder="请输入" />
       </el-form-item>
...
<script setup lang="ts">
 ...
 import {
  ...
  const getData = async () => {
    ...
    const res = await findCmsVisitor({
      id: id.value
    })
    if (res.code === 200) {
  ...
  const save = async () => {
	...
	//图片 
      ...
      if (id.value > 0) { //update
        res = await updateCmsVisitor(formData.value)
      } else {
        formData.value.status = 1
        res = await createCmsVisitor(formData.value)
      }
      if (res.code === 200) {
        console.log(res)
        beChange.value = true;
