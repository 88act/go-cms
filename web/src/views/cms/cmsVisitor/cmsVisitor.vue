		...
		<div class="gocms-table-box bg-bg_color"> 
						...
						<el-date-picker v-model="searchInfo.createdAtBetween" type="datetimerange"
							...
							<el-form-item label="id">
								...
								<el-form-item label="类型" prop="type">                
									...
									<el-option v-for="(item,key) in group_type_options" :key="key" :label="item.label" :value="item.value"></el-option>
									</el-select>
								</el-form-item>
								<el-form-item label="对象Id">
									<el-input placeholder="搜索条件" v-model="searchInfo.objId" clearable />
								</el-form-item>
								<el-form-item label="平台" prop="platType">                
									<el-select v-model="searchInfo.platType" placeholder="请选择" clearable>
									<el-option v-for="(item,key) in plat_type_options" :key="key" :label="item.label" :value="item.value"></el-option>
									</el-select>
								</el-form-item>
			...
			<el-table row-key="id" ref="multipleTable" border  style="width: 100%" tooltip-effect="dark" :data="tableData" @selection-change="handleSelectionChange" @sort-change="sortChange" >
				...
				<el-table-column label="序号" width="80" prop="id" sortable="custom" /> 
					...
					<el-table-column label="类型" prop="type" min-width="120"  sortable="custom" >
					...
					<el-popover trigger="click" placement="top"  width = "280">  
						...
						<template #reference>
							<div class="quickEdit" > {{filterDict(scope.row.type,group_type_options)}} </div>
						</template>
						</el-popover>
					</template>  
					</el-table-column> 
					<el-table-column label="对象Id" prop="objId" min-width="120"   sortable="custom"  />
					<el-table-column label="平台" prop="platType" min-width="120"  sortable="custom" >
					<template #default="scope">  
					<el-popover trigger="click" placement="top"  width = "280">  
						<el-select v-model="scope.row.platType" placeholder="请选择"  @change="quickEdit_do('plat_type',scope.row.id,scope.row.platType,scope)">
							<el-option v-for="(item,key) in plat_type_options" :key="key" :label="item.label" :value="item.value"></el-option>
						</el-select> 
						<template #reference>
							<div class="quickEdit" > {{filterDict(scope.row.platType,plat_type_options)}} </div>
						</template>
						</el-popover>
					</template>  
					</el-table-column> 
					<el-table-column label="客户端ip" prop="clientIp" min-width="120"   sortable="custom"  />
...
<script setup lang="tsx">
 ...
 import CmsVisitorForm from './cmsVisitorForm.vue'
   ...
   const multipleSelection = ref([])  
    ...
    // 字典 
		const group_type_options = ref([]) 
		const plat_type_options = ref([]) 
	...
	const deleteRow = async (row) => {
		ElMessageBox.confirm('确认删除?', '提示', {
			...
			.then(async () => {
				...
				const res = await deleteCmsVisitorByIds(data)
				if (res.code === 200) {
	...
	const getTableData = async () => {
		...
		const res = await getCmsVisitorList(paramData)
		if (res.code === 200) {
	...
	const getOptionsData = async () => { 
			group_type_options.value = await getDict('group_type') 
			plat_type_options.value = await getDict('plat_type') 
