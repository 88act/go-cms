<template>
        ...
        <el-form-item label="总粉丝:"  prop="totalFan">
                 <el-input v-model.number="formData.totalFan" clearable placeholder="请输入" />
       </el-form-item>
  ...
  const save = async () => {
	...
	//图片
      ...
      if (id.value > 0) { //update
        ...
        res = await createCmsBlog(formData.value)
      }
      if (res.code === 200) {
        console.log(res)
        beChange.value = true;
