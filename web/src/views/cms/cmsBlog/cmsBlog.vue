		...
		<div class="gocms-table-box bg-bg_color"> 
						...
						<el-date-picker v-model="searchInfo.createdAtBetween" type="datetimerange"
							...
							<el-form-item label="id">
								...
								<el-form-item label="地区id">
									<el-input placeholder="搜索条件" v-model="searchInfo.areaId" clearable />
								</el-form-item> 
								...
								<el-form-item label="总粉丝">
									<el-input placeholder="搜索条件" v-model="searchInfo.totalFan" clearable />
								</el-form-item>
			...
			<el-table row-key="id" ref="multipleTable" border  style="width: 100%" tooltip-effect="dark" :data="tableData" @selection-change="handleSelectionChange" @sort-change="sortChange" >
				...
				<el-table-column label="序号" width="80" prop="id" sortable="custom" /> 
					...
					<el-table-column label="简介" prop="desc" min-width="120"   sortable="custom"  />
					<el-table-column label="缩略图" prop="image" min-width="120"   sortable="custom" >
						<template #default="scope"> 						    
							 <FileListView :objList="getFileByGuidStr(scope.row.image,scope.row.fileObjList)" />  
						</template>
					</el-table-column>
					<el-table-column label="文件列表" prop="fileList" min-width="120"   sortable="custom" >
						<template #default="scope"> 						    
...
<script setup lang="tsx">
 ...
 import CmsBlogForm from './cmsBlogForm.vue'
   ...
   const multipleSelection = ref([])  
	...
	const deleteRow = async (row) => {
		ElMessageBox.confirm('确认删除?', '提示', {
			...
			.then(async () => {
				...
				const res = await deleteCmsBlogByIds(data)
				if (res.code === 200) {
	...
	const getTableData = async () => {
		...
		const res = await getCmsBlogList(paramData)
		if (res.code === 200) {
