...
#### 1. 前往 主程序下的initialize/router.go 在Routers 方法最末尾按照你需要的及安全模式添加本插件
    例：
    本插件可以采用gva的配置文件 也可以直接写死内容作为配置 建议为gva添加配置文件结构 然后将配置传入
	PluginInit(PrivateGroup, email.CreateEmailPlug(
		global.REDIS.Email.To,
		global.REDIS.Email.From,
		global.REDIS.Email.Host,
		global.REDIS.Email.Secret,
		global.REDIS.Email.Nickname,
		global.REDIS.Email.Port,
		global.REDIS.Email.IsSSL,
		))
