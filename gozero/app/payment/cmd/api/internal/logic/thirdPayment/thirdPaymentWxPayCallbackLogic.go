...
func (l *ThirdPaymentWxPayCallbackLogic) verifyAndUpdateState(notifyTrasaction *payments.Transaction) error {
	...
	if payStatus == model.PayStatus_PayOk {
		//Payment Notification.

		if paymentResp.PaymentDetail.PayStatus != model.PayStatus_WaitPay {
			return nil
		}
	...
	} else if payStatus == model.PayStatus_WaitPay {
...
func (l *ThirdPaymentWxPayCallbackLogic) getPayStatusByWXPayTradeState(wxPayTradeState string) int32 {

	switch wxPayTradeState {
	case SUCCESS: //支付成功
		return model.PayStatus_PayOk
	case USERPAYING: //支付中
		return model.PayStatus_WaitPay
	case REFUND: //已退款
		return model.PayStatus_WaitPay
	default:
		return model.PayStatus_Fail
	}
}
