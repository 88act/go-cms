			...
			<el-form :inline="true" :model="searchInfo" class="demo-form-inline">
				<el-form-item label="创建时间">
					<el-date-picker v-model="searchInfo.createdAtBetween" type="datetimerange"
						format="YYYY-MM-DD HH:mm:ss" :shortcuts="shortcuts" range-separator="至" start-placeholder="开始日期"
			...
			<el-table-column label="路径" prop="url" width="120" sortable="custom">
				<template #default="scope">
					<el-popover trigger="click" placement="top" width="280">
						<el-row :gutter="10">
							<el-col :span="16">
								<el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.url">
								</el-input>
							</el-col>
							<el-col :span="4">
								<el-button size="small" type="primary" icon="el-icon-edit" class="table-button"
									@click="quickEdit_do('url',scope.row.ID,scope.row.url,scope)">保存</el-button>
							</el-col>
						</el-row>
						<template #reference>
			...
			<el-table-column label="开始页码" prop="pageStart" width="120">
				<template #default="scope">
					<el-popover trigger="click" placement="top" width="280">
						<el-row :gutter="10">
							<el-col :span="16">
								<el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.pageStart">
								</el-input>
							</el-col>
							<el-col :span="4">
								<el-button size="small" type="primary" icon="el-icon-edit" class="table-button"
									@click="quickEdit_do('page_start',scope.row.ID,scope.row.pageStart,scope)">保存
								</el-button>
							</el-col>
						</el-row>
						<template #reference>
			...
			<el-table-column label="结束页码" prop="pageEnd" width="120">
				<template #default="scope">
					<el-popover trigger="click" placement="top" width="280">
						<el-row :gutter="10">
							<el-col :span="16">
								<el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.pageEnd">
								</el-input>
							</el-col>
							<el-col :span="4">
								<el-button size="small" type="primary" icon="el-icon-edit" class="table-button"
									@click="quickEdit_do('page_end',scope.row.ID,scope.row.pageEnd,scope)">保存
								</el-button>
							</el-col>
						</el-row>
						<template #reference>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async doDelete(ids) {
				const res = await deleteColCollectByIds({
					ids
				})
				if (res.code === 200) {
					this.$message({
			...
			async goEditForm(id) {
				if (this.beNewWindow) {
					...
					if (id > 0) {
						const res = await findColCollect({
							ID: id
						})
